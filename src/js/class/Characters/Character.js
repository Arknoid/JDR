 ...
 */
  ...
  import Weapon from 'src/js/class/Weapons/Weapon';
...
class Character {
  constructor(obj) {
    ...
    // Assignment of a basic weapon object
    this.rightHand = new Weapon(data.items.weapons.hand);
    this.leftHand = new Weapon(data.items.weapons.hand);

    this.twoHandedWeapon = false;
    this.block = 1;
    this.block += this.leftHand.toParry;
    this.block += this.rightHand.toParry;

    this.damage = this.rightHand.damage;
    this.numberAttack = 0;
    this.skillsAttack = [];
    ...
    if (obj.leftHand != "") { 
      let thisData = data;
      this.leftHand = new Weapon(eval('thisData.items.weapons.'+obj.leftHand));
      this.skillsAttack.push(this.leftHand);
      this.numberAttack ++;
    }
    if (obj.rightHand != "") {
      let thisData = data;
      this.rightHand = new Weapon(eval('thisData.items.weapons.'+obj.rightHand));
      this.skillsAttack.push(this.rightHand);
      this.numberAttack ++;
    }
  ...
  //Resume timer when the game is resumed
  resumeTimers() {
    $.each(this.skillsTimers, function (index, element) {
      element.start();
  ...
  //Attack target
  attack(target,handWeapon) {

    let toHit = eval("this."+handWeapon+".toHit");
    let damage = eval("this."+handWeapon+".damage");

    this.animateAttack();
    const rndDamage = randomNumber(1,damage);

    //Test if Shield Block
    if (target.shieldUp) {
      //Shield block sound
      app.game.soundsController.play('shieldBlock')
      target.showBlock();
      target.shieldUp = false;

      //Test if touch !
    } else if (randomNumber(1,toHit) > randomNumber(1, target.block)) {
      ...
      app.game.soundsController.play(this.soundMiss[randomNumber(0, this.soundMiss.length - 1)]);
      target.showBlock();

    } else {
      //play parry sound
      app.game.soundsController.play('swordBlock')
      target.showBlock();
    }
  };
  ...
  scream() {
    const rndSound = randomNumber(0, this.soundVoiceHit.length - 1)
    app.game.soundsController.play(this.soundVoiceHit[rndSound]);
  }
  useSkill(evt) {
    ...
    if (evt.data.self.canUseSkills && !app.paused) {
      ...
      if (skill.data('canUse')) {
        switch (skill.data('type')) {
          case 'attack':
            app.game.combatManager(skill.data('owner'),skill.data('hand'));
            disableTimer = skill.data('speed');
            break;
          case 'shield':
            self.shieldUp = true;
            app.game.soundsController.play('shieldUp');
            disableTimer = skill.data('speed');
            break;
          default:
        }
  ...
  dies() {
    const self = this;
    this.isDie = true;
    app.game.soundsController.play(this.soundDie[randomNumber(0, this.soundDie.length - 1)]);
    $('#' + this.id).fadeOut('slow', function() {
      $(this).remove();
      if (self.id === 'player1' || self.id === 'player2') {
        app.game.playerManager();
      } else {

        self.clearAutoAttack();
        app.game.enemyManager();
      }
    });
  }
  updateStats() {
  ...
  generateHtml() {
    ...
    const divLife = $('<div>').text(this.life)
      ...
      .attr("title", "Health");
    const divToBlock = $('<div>').text(this.block)
      .addClass('card-toHit')
    ...
    if (this.twoHandedWeapon === false) {
        //generate left Hand 
        $('<div>')
      .addClass('card-attack skill skill--img-attack')
      .data('owner', this.id)
      .data('hand', 'leftHand')
      .data('damage', this.leftHand.damage)
      .data('toHit', this.leftHand.toHit)
      .data('toParry', this.leftHand.toParry)
      .data('speed', this.leftHand.speed)
      .data('canUse', true)
      .data('type', 'attack')
      ...
      if (this.shield != 'shield') {
        ...
        .data('owner', this.id)
        .data('hand', 'rightHand')
        .data('damage', this.rightHand.damage)
        .data('toHit', this.rightHand.toHit)
        .data('toParry', this.rightHand.toParry)
        .data('speed', this.rightHand.speed)
        .data('canUse', true)
        .data('type', 'attack')
        .attr("data-toggle", "tooltip").attr("title", "Attack enemy")
        .on('click',{self:self},self.useSkill)
